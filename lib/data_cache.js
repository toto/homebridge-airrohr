...
class DataCache {
  ...
  updateViaAPI(airQualityUrl, temperatureSensorUrl, callback) {
    async.parallel(
      [
        (cb) => loadCurrentSensorData(airQualityUrl, cb),
        (cb) => loadCurrentSensorData(temperatureSensorUrl, cb)
      ],
      (error, result) => {
        if (error) {
          callback(error);
          ...
          updateAirQuality(result[0]);
          updateHumidity(result[1]);
          updateTemperature(result[1]);
        }
      }
    );
  }
  ...
  updateFromLocalSensor(url, callback) {
    loadCurrentSensorData(url, (error, json) => {
      if (error) {
        callback(error);
        return;
      }
  ...
  _updateHumidity(json) {
    ...
    for (let key of tempKeys) {
      const value = findValue(json, key);
      if (value) {
        this.humidity = value;
        break;
      }
  ...
  _updateTemperature(json) {
    ...
    for (let key of tempKeys) {
      const value = _findValue(json, key);
      if (value) {
  ...
  _updateAirQuality(json) {
    const pm25keys = ['SDS_P2'];
    const pm10keys = ['SDS_P1'];
